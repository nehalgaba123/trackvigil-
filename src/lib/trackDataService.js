...
export const STEP = 0.1; // 100 m resolution, sample dataset only

// GPS column aliases — order-independent, case-insensitive header matching.
// Add more aliases here if upstream TRC exports use different column names.
const LAT_HEADERS = [
  "latitude",
  "lat",
  "gps_lat",
 ...
 * ascending. Needed by Snapshot Comparison's two date pickers.
 * ------------------------------------------------------------------------- */
...
export function computeSnapshotDiff(
  ...
  atDate.forEach((byDate, k) => {
    if (!(dateA in byDate) || !(dateB in byDate)) return;

    const [chainage, param] = k.split("|");

    const from = classifySeverity(
      param,
      byDate[dateA],
      thresholds
    );

    const to = classifySeverity(
      param,
      byDate[dateB],
      thresholds
    );
...
const SAMPLE_SECTIONS_META = [
  {
    ...
    label: "Gauge widening — curve exit",
    zone: "Sector A",
  },
  {
    id: "c2",
    ...
    label: "Alignment drift — embankment",
    zone: "Sector B",
  },
  {
    id: "c3",
    ...
    width: 0.4,
    peakSeverity: "critical",
    label: "Cross-level fault — level crossing",
    zone: "Sector C",
  },
  {
    id: "c4",
    ...
    width: 1.2,
    peakSeverity: "warning",
    label: "Rail wear zone — high-tonnage section",
    zone: "Sector D",
  },
  {
    id: "c5",
  ...
  },
  {
    ...
    width: 0.7,
    peakSeverity: "warning",
    label: "Unevenness patch — bridge approach",
    zone: "Sector F",
  },
];
...
function buildSampleCsvText() {
  ...
  const n =
    Math.round(
      (CHAINAGE_END - CHAINAGE_START) / STEP
    ) + 1;

  for (let i = 0; i < n; i++) {
    const chainage = +(
      CHAINAGE_START + i * STEP
    ).toFixed(1);

    PARAM_KEYS.forEach((param) => {
      const warn =
        DEFAULT_THRESHOLDS[param].warning;

      const crit =
        DEFAULT_THRESHOLDS[param].critical;
      ...
      SAMPLE_SECTIONS_META
        .filter((c) => c.param === param)
        .forEach((c) => {
          const target =
            c.peakSeverity === "critical"
              ? crit * 1.18
              : warn * 1.32;
      ...
      rows.push(
        `${chainage},${CURRENT_PASS_DATE},${param},${value}`
      );
    });
  }
  ...
  SAMPLE_SECTIONS_META.forEach((section) => {
    const warn =
      DEFAULT_THRESHOLDS[section.param].warning;

    const crit =
      DEFAULT_THRESHOLDS[section.param].critical;

    const target =
      section.peakSeverity === "critical"
        ? crit * 1.18
        : warn * 1.32;
    ...
    for (let m = -7; m <= -1; m++) {
      ...
      rows.push(
        `${section.center},${dateStr},${section.param},${value.toFixed(2)}`
      );
    }
  });

  rows.push(
    `14.2,${CURRENT_PASS_DATE},guage,4.1`
  );

  rows.push(
    `,${CURRENT_PASS_DATE},twist,2.0`
  );
  ...
  rows.push(
    `31.5,${CURRENT_PASS_DATE},railWear,not_a_number`
  );

  rows.push(
    `45.0,${CURRENT_PASS_DATE},crossLevel`
  );

  rows.push(
    `9.9,${CURRENT_PASS_DATE},unevenness,`
  );

  rows.push(
    `bad_chainage,${CURRENT_PASS_DATE},gauge,2.2`
  );
 ...
 * GPS columns (latitude / longitude) are optional — detected by header name,
 * carried through to valid rows, and later lifted into wide readings by
 * longToWide(). Existing files with no GPS columns are unaffected.
 * ------------------------------------------------------------------------- */
export function parseLockedCsv(text) {
  const lines = text.trim().split(/\r?\n/);

  // Detect optional GPS columns from the header row.
  const headers = lines[0]
    .split(",")
    .map((h) => h.trim().toLowerCase());

  const latCol = headers.findIndex((h) =>
    LAT_HEADERS.includes(h)
  );

  const lngCol = headers.findIndex((h) =>
    LNG_HEADERS.includes(h)
  );
  ...
  dataLines.forEach((line, i) => {
    ...
    if (
      chainageRaw === "" ||
      Number.isNaN(chainage)
    ) {
      invalid.push({
        row: rowNum,
        raw: line,
        reason: "invalid chainage",
      });
    ...
    if (!PARAM_KEYS.includes(paramRaw)) {
      invalid.push({
        row: rowNum,
        raw: line,
        reason: `unknown parameter "${paramRaw}"`,
      });
    ...
    valid.push({
      ...
      // GPS — null when column absent or value non-numeric
      latitude:
        latCol !== -1
          ? parseFloat(cols[latCol]) || null
          : null,

      longitude:
        lngCol !== -1
          ? parseFloat(cols[lngCol]) || null
          : null,
    });
  });
 ...
 * Also accepts optional latitude / longitude fields on each row.
 * ------------------------------------------------------------------------- */
      ...
      invalid: [
        {
          row: 0,
          raw: "",
          reason: "JSON root must be an array of rows",
        },
      ],
      total: 1,
    };
  ...
  arr.forEach((row, i) => {
    ...
    if (!PARAM_KEYS.includes(row?.parameter)) {
      invalid.push({
        row: rowNum,
        raw: JSON.stringify(row),
        reason: `unknown parameter "${row?.parameter}"`,
      });
    ...
    valid.push({
      ...
      // GPS — null when absent or non-numeric
      latitude:
        row.latitude != null
 ...
 *     latitude, longitude }
 *
 * latitude / longitude are null when the source file has no GPS columns.
 *
...
export function longToWide(validRows) {
  ...
  validRows
    ...
    .forEach((r) => {
      if (!byChainage.has(r.chainage)) {
        byChainage.set(r.chainage, {
          chainage: r.chainage,
          latitude: null,
          longitude: null,
        });
      ...
      const entry = byChainage.get(r.chainage);

      entry[r.parameter] = r.value;

      // Take the first non-null GPS value seen for this chainage point.
      if (
        entry.latitude == null &&
        r.latitude != null
      ) {
...
/* ---------------------------------------------------------------------------
 * buildTrendSectionsFromSample — sample-dataset path, UNCHANGED.
 * ------------------------------------------------------------------------- */
export function buildTrendSectionsFromSample(
  validRows,
  currentDate,
  wideReadings
) {
  const historical = validRows.filter(
  ...
  SAMPLE_SECTIONS_META.forEach((meta) => {
    const rows = historical
      .filter(
        (r) =>
          r.parameter === meta.param &&
          Math.abs(
            r.chainage - meta.center
          ) < 0.05
      )
      .sort(
        (a, b) =>
...
/* ---------------------------------------------------------------------------
 * buildTrendSectionsGeneric — upload / API path, UNCHANGED.
 * ------------------------------------------------------------------------- */
export function buildTrendSectionsGeneric(
  validRows,
  currentDate,
  wideReadings,
  thresholds,
 ...
 * Readings carry latitude: null / longitude: null automatically because
 * buildSampleCsvText() emits no GPS columns and longToWide() now always
 * initialises both fields — no change needed here.
 * ------------------------------------------------------------------------- */
export function loadSampleDataset() {
  ...
  return {
    ...
    meta: {
      ...
      note:
        "Synthetic data generated for threshold-based monitoring demonstration — not live TRC data.",
 ...
 * GPS columns detected automatically by parseLockedCsv / parseLockedJson
 * and propagated into readings by longToWide — nothing extra needed here.
 * ------------------------------------------------------------------------- */
export function loadFromUpload(
  text,
  filename
) {
  const isJson =
  ...
  return {
    ...
    meta: {
      ...
      note:
        "Anomaly detection and statistical trend analysis run against uploaded inspection data.",
...
function normalizeApiAlerts(
  alertsJson
) {
  const rawList =
    Array.isArray(alertsJson)
      ? alertsJson
      : Array.isArray(
          alertsJson?.alerts
        )
...
/* ---------------------------------------------------------------------------
 * PUBLIC: loadFromApi()
 * Readings built directly here (not via longToWide), so latitude / longitude
 * are explicitly set to null. When the backend starts supplying GPS data,
 * map t.latitude / t.longitude from the /tracks response into these fields.
 * ------------------------------------------------------------------------- */
export async function loadFromApi(
  ...
  if (!tracksRes.ok) {
    throw new Error(
      `Backend GET /tracks failed (${tracksRes.status}). Is the server running on ${baseUrl}?`
    );
  }
  ...
  if (tracks.length === 0) {
    throw new Error(
      "Backend /tracks returned no data — the server may not have a processed dataset yet."
    );
  }
  ...
  const readings = tracks
    .map((t) => {
      const row = {
        ...
        // GPS — wire in t.latitude / t.longitude
        // once backend supplies them
        latitude:
          t.latitude ?? null,
  ...
  return {
    ...
    meta: {
      ...
      note:
        `Connected to TrackVigil backend at ${baseUrl}.`,
